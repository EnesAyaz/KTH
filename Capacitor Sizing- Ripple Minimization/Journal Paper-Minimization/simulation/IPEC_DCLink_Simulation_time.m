...
%% =========================================================================
%  SECTION 7: TIME-DOMAIN WAVEFORMS
%  Reconstructs full-resolution waveforms over 2 fundamental cycles:
%    (1) Phase voltages  – pulsating PWM pole voltages Van, Vbn, Vcn
%    (2) Phase currents  – sinusoidal with switching ripple (RL load)
%    (3) DC-link input current  Iinv(t) = ia*Sa + ib*Sb + ic*Sc
%    (4) DC-link capacitor current  Icap(t) = Iinv(t) - Idc
%  Plotted for both Conventional and Proposed PWM on the same axes.
%  =========================================================================

fprintf('=== Building Time-Domain Waveforms ===\n');

% --- Simulation parameters ---
N_cycles  = 2;                         % Number of fundamental cycles to show
Npts_td   = 200;                       % Time points per switching period
N_total   = N_cycles * Nsw * Npts_td;  % Total time points
t_total   = linspace(0, N_cycles*T1, N_total);
dt_td     = t_total(2) - t_total(1);

% Operating point (reuse ma, pf_angle from Section 4)
% Phase currents are computed by integrating the RL load equation:
%   L*di/dt + R*i = V_phase(t)  using forward Euler

% Pre-allocate waveform arrays
Van_conv = zeros(1,N_total);  Van_prop = zeros(1,N_total);
Vbn_conv = zeros(1,N_total);  Vbn_prop = zeros(1,N_total);
Vcn_conv = zeros(1,N_total);  Vcn_prop = zeros(1,N_total);

ia_conv  = zeros(1,N_total);  ia_prop  = zeros(1,N_total);
ib_conv  = zeros(1,N_total);  ib_prop  = zeros(1,N_total);
ic_conv  = zeros(1,N_total);  ic_prop  = zeros(1,N_total);

Iinv_conv = zeros(1,N_total); Iinv_prop = zeros(1,N_total);
Icap_conv_td = zeros(1,N_total); Icap_prop_td = zeros(1,N_total);

% Initial currents
ia_c = 0; ib_c = 0; ic_c = 0;
ia_p = 0; ib_p = 0; ic_p = 0;

for n = 1:N_total
    t_n    = t_total(n);
    theta  = 2*pi*f1*t_n;

    % Duty cycles at this instant
    da_n = max(0.01, min(0.99, 0.5 + 0.5*ma*cos(theta)));
    db_n = max(0.01, min(0.99, 0.5 + 0.5*ma*cos(theta - 2*pi/3)));
    dc_n = max(0.01, min(0.99, 1.5 - da_n - db_n));

    % Carrier phase (normalised time within switching period)
    tau = mod(t_n, Tsw) / Tsw;

    % --- CONVENTIONAL: single triangular carrier for all legs ---
    Sa_c = double(tau < da_n);
    Sb_c = double(tau < db_n);
    Sc_c = double(tau < dc_n);

    % --- PROPOSED: LUT-based phase shifts ---
    i_sum_n = abs(ia_p) + abs(ib_p) + abs(ic_p) + 1e-10;
    ia_h_n  = max(ia_hat_vec(1), min(ia_hat_vec(end), ia_p/i_sum_n));
    ib_h_n  = max(ib_hat_vec(1), min(ib_hat_vec(end), ib_p/i_sum_n));
    d1_n = max(1, min(N_duty, round(interp1(da_vec,1:N_duty,da_n,'linear','extrap'))));
    d2_n = max(1, min(N_duty, round(interp1(db_vec,1:N_duty,db_n,'linear','extrap'))));
    phi_b_n = interp2(ia_hat_vec, ib_hat_vec, LUT_theta_b(:,:,d1_n,d2_n)', ia_h_n, ib_h_n,'linear',0);
    phi_c_n = interp2(ia_hat_vec, ib_hat_vec, LUT_theta_c(:,:,d1_n,d2_n)', ia_h_n, ib_h_n,'linear',0);

    Sa_p = double(tau < da_n);
    Sb_p = double(mod(tau - phi_b_n/(2*pi), 1) < db_n);
    Sc_p = double(mod(tau - phi_c_n/(2*pi), 1) < dc_n);

    % --- Phase voltages (pole voltages referred to DC midpoint) [V] ---
    Van_conv(n) = (Sa_c - 0.5) * Vdc;
    Vbn_conv(n) = (Sb_c - 0.5) * Vdc;
    Vcn_conv(n) = (Sc_c - 0.5) * Vdc;

    Van_prop(n) = (Sa_p - 0.5) * Vdc;
    Vbn_prop(n) = (Sb_p - 0.5) * Vdc;
    Vcn_prop(n) = (Sc_p - 0.5) * Vdc;

    % --- Phase currents via forward Euler on RL load ---
    % v_phase = V_leg - V_neutral  (star connection, balanced)
    Vn_c = (Van_conv(n) + Vbn_conv(n) + Vcn_conv(n)) / 3;
    Vn_p = (Van_prop(n) + Vbn_prop(n) + Vcn_prop(n)) / 3;

    dia_c = ((Van_conv(n)-Vn_c) - R_load*ia_c) / L_load;
    dib_c = ((Vbn_conv(n)-Vn_c) - R_load*ib_c) / L_load;
    dic_c = ((Vcn_conv(n)-Vn_c) - R_load*ic_c) / L_load;

    dia_p = ((Van_prop(n)-Vn_p) - R_load*ia_p) / L_load;
    dib_p = ((Vbn_prop(n)-Vn_p) - R_load*ib_p) / L_load;
    dic_p = ((Vcn_prop(n)-Vn_p) - R_load*ic_p) / L_load;

    ia_c = ia_c + dia_c*dt_td;  ib_c = ib_c + dib_c*dt_td;  ic_c = ic_c + dic_c*dt_td;
    ia_p = ia_p + dia_p*dt_td;  ib_p = ib_p + dib_p*dt_td;  ic_p = ic_p + dic_p*dt_td;

    ia_conv(n) = ia_c;  ib_conv(n) = ib_c;  ic_conv(n) = ic_c;
    ia_prop(n) = ia_p;  ib_prop(n) = ib_p;  ic_prop(n) = ic_p;

    % --- DC-link input current ---
    Iinv_conv(n) = ia_c*Sa_c + ib_c*Sb_c + ic_c*Sc_c;
    Iinv_prop(n) = ia_p*Sa_p + ib_p*Sb_p + ic_p*Sc_p;
end

% --- Capacitor current = Iinv - moving average (Idc over one switching period) ---
win = round(Tsw / dt_td);
Idc_conv_ma = movmean(Iinv_conv, win);
Idc_prop_ma = movmean(Iinv_prop, win);
Icap_conv_td = Iinv_conv - Idc_conv_ma;
Icap_prop_td = Iinv_prop - Idc_prop_ma;

% Time axis in ms for readability
t_ms = t_total * 1e3;

fprintf('  Waveform generation complete.\n\n');

% -------------------------------------------------------------------------
%  FIGURE 5: Time-domain waveforms — Conventional PWM
% -------------------------------------------------------------------------
figure('Name','Time-Domain Waveforms – Conventional PWM','Position',[50 50 1100 750]);

subplot(5,1,1);
plot(t_ms, Van_conv, 'Color',[0.8 0.15 0.15],'LineWidth',0.6); hold on;
plot(t_ms, Vbn_conv, 'Color',[0.15 0.6 0.15],'LineWidth',0.6);
plot(t_ms, Vcn_conv, 'Color',[0.1 0.35 0.85],'LineWidth',0.6);
ylabel('V_{phase} [V]'); title('Phase Voltages (PWM Pole Voltages)  –  Conventional');
legend('V_{an}','V_{bn}','V_{cn}','Location','EastOutside','FontSize',8);
ylim([-Vdc*0.65 Vdc*0.65]); grid on; set(gca,'FontSize',9);
set(gca,'XTickLabel',[]);

subplot(5,1,2);
plot(t_ms, ia_conv, 'Color',[0.8 0.15 0.15],'LineWidth',0.9); hold on;
plot(t_ms, ib_conv, 'Color',[0.15 0.6 0.15],'LineWidth',0.9);
plot(t_ms, ic_conv, 'Color',[0.1 0.35 0.85],'LineWidth',0.9);
ylabel('i_{phase} [A]'); title('Phase Currents (with Switching Ripple)');
legend('i_a','i_b','i_c','Location','EastOutside','FontSize',8);
grid on; set(gca,'FontSize',9); set(gca,'XTickLabel',[]);

subplot(5,1,3);
plot(t_ms, ia_conv.*double(Van_conv > 0) + ib_conv.*double(Vbn_conv > 0) + ic_conv.*double(Vcn_conv > 0), ...
    'Color',[0.4 0.1 0.6],'LineWidth',0.8);
ylabel('I_{inv} [A]'); title('DC-Link Input Current  I_{inv}(t)');
grid on; set(gca,'FontSize',9); set(gca,'XTickLabel',[]);

subplot(5,1,4);
plot(t_ms, Iinv_conv, 'Color',[0.15 0.5 0.5],'LineWidth',0.8);
ylabel('I_{inv} [A]'); title('Inverter Input Current  (i_a S_a + i_b S_b + i_c S_c)');
grid on; set(gca,'FontSize',9); set(gca,'XTickLabel',[]);

subplot(5,1,5);
plot(t_ms, Icap_conv_td, 'Color',[0.85 0.35 0.1],'LineWidth',0.8);
yline(0,'--k','LineWidth',0.8);
ylabel('i_{cap} [A]'); xlabel('Time [ms]');
title('DC-Link Capacitor Current  i_{cap}(t)');
grid on; set(gca,'FontSize',9);

sgtitle(sprintf('Conventional Single-Carrier PWM  |  m_a=%.2f, PF=%.2f, f_{sw}=%.0fkHz, V_{dc}=%dV', ...
    ma, cos(pf_angle), fsw/1e3, Vdc), 'FontSize',11,'FontWeight','bold');

% -------------------------------------------------------------------------
%  FIGURE 6: Time-domain waveforms — Proposed Multi-Carrier PWM
% -------------------------------------------------------------------------
figure('Name','Time-Domain Waveforms – Proposed PWM','Position',[80 80 1100 750]);

subplot(5,1,1);
plot(t_ms, Van_prop, 'Color',[0.8 0.15 0.15],'LineWidth',0.6); hold on;
plot(t_ms, Vbn_prop, 'Color',[0.15 0.6 0.15],'LineWidth',0.6);
plot(t_ms, Vcn_prop, 'Color',[0.1 0.35 0.85],'LineWidth',0.6);
ylabel('V_{phase} [V]'); title('Phase Voltages (PWM Pole Voltages)  –  Proposed Multi-Carrier');
legend('V_{an}','V_{bn}','V_{cn}','Location','EastOutside','FontSize',8);
ylim([-Vdc*0.65 Vdc*0.65]); grid on; set(gca,'FontSize',9);
set(gca,'XTickLabel',[]);

subplot(5,1,2);
plot(t_ms, ia_prop, 'Color',[0.8 0.15 0.15],'LineWidth',0.9); hold on;
plot(t_ms, ib_prop, 'Color',[0.15 0.6 0.15],'LineWidth',0.9);
plot(t_ms, ic_prop, 'Color',[0.1 0.35 0.85],'LineWidth',0.9);
ylabel('i_{phase} [A]'); title('Phase Currents (with Switching Ripple)');
legend('i_a','i_b','i_c','Location','EastOutside','FontSize',8);
grid on; set(gca,'FontSize',9); set(gca,'XTickLabel',[]);

subplot(5,1,3);
plot(t_ms, ia_prop.*double(Van_prop > 0) + ib_prop.*double(Vbn_prop > 0) + ic_prop.*double(Vcn_prop > 0), ...
    'Color',[0.4 0.1 0.6],'LineWidth',0.8);
ylabel('I_{inv} [A]'); title('DC-Link Input Current  I_{inv}(t)');
grid on; set(gca,'FontSize',9); set(gca,'XTickLabel',[]);

subplot(5,1,4);
plot(t_ms, Iinv_prop, 'Color',[0.15 0.5 0.5],'LineWidth',0.8);
ylabel('I_{inv} [A]'); title('Inverter Input Current  (i_a S_a + i_b S_b + i_c S_c)');
grid on; set(gca,'FontSize',9); set(gca,'XTickLabel',[]);

subplot(5,1,5);
plot(t_ms, Icap_prop_td, 'Color',[0.1 0.5 0.8],'LineWidth',0.8);
yline(0,'--k','LineWidth',0.8);
ylabel('i_{cap} [A]'); xlabel('Time [ms]');
title('DC-Link Capacitor Current  i_{cap}(t)');
grid on; set(gca,'FontSize',9);

sgtitle(sprintf('Proposed Multi-Carrier PWM  |  m_a=%.2f, PF=%.2f, f_{sw}=%.0fkHz, V_{dc}=%dV', ...
    ma, cos(pf_angle), fsw/1e3, Vdc), 'FontSize',11,'FontWeight','bold');

% -------------------------------------------------------------------------
%  FIGURE 7: Direct comparison — capacitor current overlay + zoom
% -------------------------------------------------------------------------
figure('Name','Capacitor Current Comparison','Position',[110 110 1100 500]);

subplot(2,1,1);
plot(t_ms, Icap_conv_td, 'Color',[0.85 0.2 0.2],'LineWidth',0.7); hold on;
plot(t_ms, Icap_prop_td, 'Color',[0.1 0.5 0.8],'LineWidth',0.7);
yline(0,'--k','LineWidth',0.6);
ylabel('i_{cap} [A]'); title('DC-Link Capacitor Current: Conventional vs. Proposed (Full View)');
legend('Conventional','Proposed Multi-Carrier','Location','NorthEast','FontSize',9);
grid on; set(gca,'FontSize',10); set(gca,'XTickLabel',[]);

% Zoom into 2 switching periods around the current peak
[~,peak_idx] = max(abs(Icap_conv_td));
zoom_half = round(2*Tsw/dt_td);
z1 = max(1, peak_idx - zoom_half);
z2 = min(N_total, peak_idx + zoom_half);

subplot(2,1,2);
plot(t_ms(z1:z2), Icap_conv_td(z1:z2), 'Color',[0.85 0.2 0.2],'LineWidth',1.2); hold on;
plot(t_ms(z1:z2), Icap_prop_td(z1:z2), 'Color',[0.1 0.5 0.8],'LineWidth',1.2);
yline(0,'--k','LineWidth',0.8);
xlabel('Time [ms]'); ylabel('i_{cap} [A]');
title(sprintf('Zoom: 4 Switching Periods around Peak Current  (T_{sw}=%.0f\\mus)', Tsw*1e6));
legend('Conventional','Proposed','Location','NorthEast','FontSize',9);
grid on; set(gca,'FontSize',10);

sgtitle(sprintf('DC-Link Capacitor Current Comparison  |  Conventional RMS=%.3fA  |  Proposed RMS=%.3fA  |  Reduction=%.1f%%', ...
    rms(Icap_conv_td), rms(Icap_prop_td), ...
    (1 - rms(Icap_prop_td)/rms(Icap_conv_td))*100), ...
    'FontSize',11,'FontWeight','bold');
