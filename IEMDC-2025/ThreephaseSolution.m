...
Ld= 61.1e-6; % d-axis inductance value in H
...
%% 
if given_parameters==3
% Given parameters- 3
id = -118.36; %  d-axis current value in A
iq = 222.25; %  q-axis current value in A
vd = -198.3; %  d-axis voltage value in V
vq = 175.4; %  q-axis voltage value in V
Vdc= 625; % DC link voltage in V
Ld= 65.1e-6; % d-axis inductance value in H
Lq=243e-6; % q-axis inductance value in H
Torque= 370; % Torque of the motor in N.m
RPM = 5000; % Rotational speed in RPM
end

%% 
if given_parameters==4
% Given parameters- 4
id = -118.36; %  d-axis current value in A
iq = 222.25; %  q-axis current value in A
vd = -198.3; %  d-axis voltage value in V
vq = 175.4; %  q-axis voltage value in V
Vdc= 625; % DC link voltage in V
Ld= 66e-6; % d-axis inductance value in H
Lq=146e-6; % q-axis inductance value in H
Torque= 370; % Torque of the motor in N.m
RPM = 5000; % Rotational speed in RPM
end
...
%% Load calculation 
resistance= max(v_a)*power_factor/max(i_a);
%%
...
data.time = time;
data.i_a = i_a_differential;
data.i_b = i_b_differential;
data.i_c = i_c_differential;
data.id = id;
data.iq = iq;
data.vd = vd;
data.vq = vq;
