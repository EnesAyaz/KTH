number_of_data=1000;
Eoff_150_Id= linspace(0,700,number_of_data);
Eon_150_Id= linspace(0,700,number_of_data);

Ecoss_off=0;
Eoff_150_inter=linspace(Ecoss_off,45,number_of_data); % in mJ

a=0.25;
Eoff_150_inter =a*(Eoff_150_inter) + (1-a)*(Eoff_150_inter.^2)/45;

Ecoss_on=0;
Eon_150_inter=linspace(Ecoss_on,70,number_of_data);  % in mJ

b=0.25;
Eon_150_inter =b*(Eon_150_inter) + (1-b)*(Eon_150_inter.^2)/70;

rds_T1= linspace(25,125,21);
rds_inter1= linspace(2.1e-3,3.4e-3,21);

rds_T2= linspace(125,175,11);
rds_inter2= linspace(3.4e-3,4.3e-3,11);

rds_T=[rds_T1 rds_T2(2:end)];
rds_inter=[rds_inter1 rds_inter2(2:end)];

clear Ecoss_on Ecoss_off number_of_data rds_inter1 rds_inter2 rds_T1 rds_T2
%%
fc = 22.5e3; % Carrier frequency
% die_number=12;
%  scaling=1;
%%
...
hold(axes1,'on');
% Create plot
plot(Tjx,Ploss_con_Tj,"Marker",'x','LineWidth',3,...
    'Color',[1 0 0]);
% Create ylabel
...
hold(axes1,'on');
% Create plot
stem(fon,Eon,'DisplayName','Eon',"Marker",'x','LineWidth',3,...
    'Color',[1 0 0]);
yline(3*sum(Eon)*f1/1000,'DisplayName','P_{sw-on}','LineWidth',3,'Color',[0 0 1]);
% Create ylabel
...
hold(axes1,'on');
% Create plot
stem(fon,Eoff,'DisplayName','Eoff',"Marker",'x','LineWidth',3,...
    'Color',[1 0 0]);
yline(3*sum(Eoff)*f1/1000,'DisplayName','P_{sw-off}','LineWidth',3,'Color',[0 0 1]);
% Create ylabel
ylabel({'Energy (Joule), Switching  Loss (W)'});
% Create xlabel
xlabel({'Time (ms)'});
