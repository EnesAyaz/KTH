...
for i=1:length(gamma)
    ...
    elseif gamma(i)> gamma_2 && gamma(i) < gamma_3
     ...
     P_TVSW1=[P_TVSW1 0];
      P_TVSW2=[P_TVSW2  E_sw_ref*f1* (pI1*(i_L(i)*A_ac_die_x/(A_ac_die*n_die))^2 * pI2*(i_L(i)*A_ac_die_x/(A_ac_die*n_die))+pI3)*f3*f4*delta_gamma];
...
Pt_one_leg_fs= fs*n_die*A_ac_die/A_ac_die_x*(sum(P_TVSW1)+sum(P_TVSW2))/(2*pi);

Pt_one_leg_fs*6

%%
fs*n_die*A_ac_die/A_ac_die_x*(sum(P_TVSW1))/(2*pi)
fs*n_die*A_ac_die/A_ac_die_x*(sum(P_TVSW2))/(2*pi)
