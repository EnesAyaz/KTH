...
motor_loss= r./NumberofDie;

Pinv= (1/6+NumberofDie/2);
...
X1=NumberofDie;
YMatrix1=[motor_loss;Pinv; (Pinv+motor_loss)];
...
set(plot1(1),'DisplayName','Motor Harmonic losses','LineStyle','--');
set(plot1(2),'DisplayName','Inverter losses','LineStyle','-.');
set(plot1(3),'DisplayName','Total losses');

% Create ylabel
ylabel({'P_{fs}/ P_{fs,opt} (p.u.)'});

% Create xlabel
xlabel({'fs/ fs_{opt} (p.u.)'});
