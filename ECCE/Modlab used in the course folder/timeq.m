function [wt,v] = timeq(noofpts,pwm_angles)
% [wt,v] = timeq(noofpts,pwm_angles)
%
% Create time function v(wt) from a set of switching angles during a period
% for two-level harmonic elimination. Quarter-wave symmetry is assumed 
% i.e. the angles are only defined during one quarter of the fundamental
% cycle. Due to the quarter-wave symmetry, the number of elements in v and
% wt is four times the input noofpts.
%
...
%
    for k=1:2:noofangles-1
        vq( find((pwm_angles(k)<wtq) & (pwm_angles(k+1)>wtq)) )=0;
    end
