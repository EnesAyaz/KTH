...
sample_theta=1e-2;
for theta=theta_pf:sample_theta:pi
    IL= I_peak*sin(theta-theta_pf);
    D_upper_top= m_a*sin(theta);
    P_c= P_c+ (r_on*IL^2*D_upper_top*sample_theta);
end

for theta=theta_pf:sample_theta:pi
    IL= I_peak*sin(theta-theta_pf);
    D_upper_middle= 1;
    P_c= P_c+ (r_on*IL^2*D_upper_middle*sample_theta);
end

for theta=pi:sample_theta:pi+theta_pf
    IL= I_peak*sin(theta-theta_pf);
    D_upper_middle= 1+m_a*sin(theta);
    P_c= P_c+ (r_on*IL^2*D_upper_middle*sample_theta);
end

for theta=pi:sample_theta:pi+theta_pf
    IL= I_peak*sin(theta-theta_pf);
    D_lower_middle= -m_a*sin(theta);
    P_c= P_c+ (r_on*IL^2*D_lower_middle*sample_theta);
end

for theta=pi:sample_theta:pi+theta_pf
    IL= I_peak*sin(theta-theta_pf);
    D_lower_bottom= -m_a*sin(theta);
    P_c= P_c+ (r_on*IL^2*D_lower_bottom*sample_theta);
end
...
u_o= 0.8117;
U_b=0.85; % kV 
r_d=(alpha_d* U_b^(2.5)+beta_d)/A_die;


for theta=theta_pf:sample_theta:pi
    IL= I_peak*sin(theta-theta_pf);
    D_upper_diode= 1-m_a*sin(theta);
    P_c= P_c+ ((r_d*IL^2+ u_o*IL)*D_upper_diode*sample_theta);
end
...
for theta=pi:sample_theta:(theta_pf+pi)
    E_zcs= E_zcs+ Esw;
end
...
% for targetEfficiency = 99.4:2:99.8
% % Specify the desired efficiency value
% % Plot a specific contour line for the desired efficiency value
% contour(X, Y, 100 * Pout ./ (Pout + P_tot), [targetEfficiency, targetEfficiency], 'LineColor', 'k', 'LineWidth', 2);
% end
