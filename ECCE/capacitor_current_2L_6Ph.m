...
thetac=0; % carrier phase offset
theta0=pi/3; % reference phase offset
[vp_d,wt,carr,ref] = mod_2lcarr(ma, pn,  npoints ,carrytype,smp,cmode,theta0,thetac,start_angle,end_angle,ma_dc); 
theta0=3*pi/3; % reference phase offset
[vp_e,wt,carr,ref] = mod_2lcarr(ma, pn,  npoints ,carrytype,smp,cmode,theta0,thetac,start_angle,end_angle,ma_dc); 
  theta0=5*pi/3; % reference phase offset
[vp_f,wt,carr,ref] = mod_2lcarr(ma, pn,  npoints ,carrytype,smp,cmode,theta0,thetac,start_angle,end_angle,ma_dc); 
...
ippk = P*2/6/uppk/cosphi;  % Peak phase current
...
ip_d = ippk*cos(wt-phi+pi/3);   % sampled phase current over one cycle
ip_e = ippk*cos(wt-phi +3*pi/3);   % sampled phase current over one cycle
ip_f = ippk*cos(wt-phi +5*pi/3);   % sampled phase current over one cycle
%%
...
%%
Ic=Isw_a+Isw_b+Isw_c+Isw_d+Isw_e+Isw_f;
Ic=Ic-mean(Ic);
