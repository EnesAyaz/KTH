...
%%
i = 29000;
time_diff_fft = time_data(i:i+2000000);
Vdiff_fft = phA_smooth - phB_smooth;
Vdiff_fft = Vdiff_fft(1:length(time_diff_fft));

% --- FFT parameters ---
Fs = 1 / (time_diff_fft(2) - time_diff_fft(1));   % Sampling frequency
N  = length(Vdiff_fft);                           % Number of samples

% --- FFT computation ---
Y = fft(Vdiff_fft);                % FFT result
P2 = abs(Y/N);                     % Two-sided spectrum
P1 = P2(1:floor(N/2)+1);           % Single-sided spectrum
P1(2:end-1) = 2*P1(2:end-1);       % Scale for single-sided
...
ylabel('Normalized Line-to-Line Voltage', 'FontName','Times New Roman','FontSize',15);
grid on;
...
xlim([1 1e5]);   % 1 kHz to 1000 kHz (1 MHz)

set(gca,'FontName','Times New Roman','FontSize',18);

%% 
THD= 100*(sqrt(sum(P1(3:1000).^2))/0.5)
%%
% indices corresponding to harmonics (example 3:1000 bins, but better to pick multiples of f0!)
idx = 3:1000;
w   = 1./idx;                        % weights = 1/h

% Apply weights correctly
WTHD = 100 * ( sqrt ( sum( (P1(idx) .* w').^2 ) ) / 0.5 )
