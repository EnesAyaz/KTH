...
%     S(i)=S1(i)-S2(i)-S3(i)-S2(i);   
%      S(i)= abs(S1(i)+S3(i))-abs(S2(i));
...
hold on;
plot(theta_f1*180/pi,abs(S1+S2+S3),'color',[0.5 0 0])

x = min(abs(S1+S2+S3))*ones(size(theta_f1));
...
plot(theta_f1*180/pi,S1,'color',[0 0 0.5])
hold on;
plot(theta_f1*180/pi,S2,'color',[0.5 0 0])
hold on;
plot(theta_f1*180/pi,S3,'color',[0 0.5 0])
hold on;
...
legend1 = legend(axes1,'show',{'S1','S2','S3'},'interpreter','Latex','FontName','TimesNewRoman','FontSize',10);
set(legend1,...
    'Location','Best',...
    'EdgeColor','none',...
    'Color','white');
