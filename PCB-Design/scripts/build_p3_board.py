...
zone('DC-',p.In1_Cu,[(7,7),(88,7),(88,57),(7,57)])
zone('DC+',p.In2_Cu,[(7,7),(88,7),(88,57),(7,57)])
zone('AC',p.B_Cu,[(40,32),(45,32),(45,49),(69,49),(69,29),(87,29),(87,56),(40,56)])
zone('AC',p.B_Cu,[(52.8,33.8),(55.2,33.8),(55.2,50),(52.8,50)])
...
tr('DC+',73,28,71.25,28.1,.25)
for a,c in [((5,5),(90,5)),((90,5),(90,60)),((90,60),(5,60)),((5,60),(5,5))]:line(a,c)
text('EPC2361 / TWO PER SWITCH / P3',59,8)
text('24 x 1uF 100V / ALL CAPS TOP',59,11)
text('DC+',14,23);text('DC-',33,25);text('AC',80,27)
text('REVIEW PROTOTYPE',47,58)
# Place all reference and pin labels on unobstructed silk using conservative boxes.
occupied=[]
for drawing in b.GetDrawings():
    if drawing.GetLayer()==p.F_SilkS:
        bb=drawing.GetBoundingBox();occupied.append((p.ToMM(bb.GetX())-.25,p.ToMM(bb.GetY())-.25,p.ToMM(bb.GetRight())+.25,p.ToMM(bb.GetBottom())+.25))
for f in fps.values():
...
def label(s,x,y):
    w=max(1.25,len(s)*.8+.4);h=1.35
    candidates=[(dx,dy) for dx in range(-24,25) for dy in range(-24,25)]
...
for ref,f in fps.items():
    ...
    if ref in ['JDC1','JDC2']:label(ref,x,y+7)
    elif ref=='JAC1':label(ref,x,y+7)
    elif ref in bus_caps:label(ref,x,y-4 if y<35 else y+4)
