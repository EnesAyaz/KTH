...
for j,ref in enumerate(['JDC1','JDC2','JAC1']):put(ref,30+j*48,247)
for j,ref in enumerate(['TP'+str(i) for i in range(1,14)]):put(ref,180+(j%8)*29,243+(j//8)*20)
put('LK1',330,267)
put('LK2',375,267)
...
(OUT/(NAME+'.kicad_sch')).write_text('\n'.join(['(kicad_sch (version 20230121) (generator eeschema)',f'(uuid {sheet})','(paper "A3")','(title_block (title "Four EPC2361 onboard-driver prototype") (rev "P7") (date "2026-09-22"))','(lib_symbols '+''.join(defs)+')',*items,'(sheet_instances (path "/" (page "1")))',')']))
