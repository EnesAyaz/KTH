...
def label(s,x,y,angle=0,size=.8,fixed=False):
    ...
    for a,c in candidates:
        ...
        if bb[0]<5.6 or bb[2]>89.4 or bb[1]<5.6 or bb[3]>59.4:continue
...
label('EPC2361 / TWO PER SWITCH / P3',59,8,size=1,fixed=True)
label('24 x 1uF 100V / ALL CAPS TOP',59,11,size=.9,fixed=True)
label('REVIEW PROTOTYPE',47,58,size=.9,fixed=True)
label('DC+',14,22,size=1);label('DC-',32,24,size=1);label('AC',80,27,size=1)
...
for r,c in parts.items():
    if r in bus_caps or r in ['JGH1','JGL1','JGH2','JGL2']:continue
    x,y=c['x'],c['y'];label(r,x,y+(7 if r in ['JDC1','JDC2','JAC1'] else 1.8))
    if r.startswith('TP'):label(c['value'],x,y-1.5)
