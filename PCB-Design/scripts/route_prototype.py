...
# Driver split thermal pads are never joined:20=DC-,21=AC.
tr('DC-',*pos('U1',20),*pos('U1',3),.3);tr('DC-',*pos('U1',20),*pos('U1',7),.2)
tr('AC',*pos('U1',21),*pos('U1',13),.2);tr('AC',*pos('U1',21),*pos('U1',14),.2);tr('AC',*pos('U1',21),*pos('U1',9),.2);tr('AC',*pos('U1',21),*pos('U1',16),.2)
for n,x,y in [('VIN',42.1,34),('VDD',41,35.8),('VDD',43.75,37.6),('LO',44.75,37.6),('HO',46.25,37.6),('HB',48.1,35.5),('BST',44.75,32.3),('DC-',41.8,35),('DC-',41.8,36.8)]:via(n,x,y)
for n,pin,points in [('VIN',2,[(42.5,34.5),(42.1,34)]),('VDD',4,[(42.5,35.5),(42.1,35.8),(41,35.8)]),('DC-',5,[(42.5,36),(41.8,36.8)])]:
    a=pos('U1',pin)
    for c in points:tr(n,*a,*c);a=c
tr('HI',44.25,32.6,42.8,32.6);tr('HI',42.8,32.6,42.8,32.3);via('HI',42.8,32.3)
tr('LI',43.75,33.1,42,33.1);via('LI',42,33.1)
tr('AC',47.4,35,47.4,34.5);via('AC',49.2,34.5)
# Join the remaining same-net pin escape stubs.
tr('AC',*pos('C18',2),46.25,32.3);tr('AC',46.25,32.3,*pos('U1',16));tr('AC',46.25,32.3,*pos('JGH2',2))
tr('AC',*pos('C17',2),45.5,27.225);tr('AC',45.5,27.225,45.5,29.5);tr('AC',45.5,29.5,*pos('C18',2))
for ref,x,y in [('RGH1',38.81,40.3),('RGL1',38.81,34.3),('RGH2',51.19,31.5),('RGL2',51.19,35.7)]:escape(ref,1,x,y)
for ref,pin,x,y in [('C13',1,38.225,22.7),('C13',2,39.775,22.7),('C14',1,42.6,45.45),('C14',2,42.6,43.55),('C15',1,48.4,45.45),('C15',2,48.4,43.55),('C16',1,42.8,40.275),('C16',2,42.8,38.725),('C17',1,48.2,28.775),('C18',1,44.225,30.1),('RB1',1,42.8,28.325),('RB1',2,42.8,26.675),('D1',1,45.6,22.5),('D1',2,49,22.5),('RH1',1,35,21.51),('RH1',2,35,20.49),('RL1',1,40,21.51),('RL1',2,40,20.49)]:escape(ref,pin,x,y)
for pin in ['1','3','5']:nodes.setdefault(parts['JCTRL1']['pins'][pin],[]).append(pos('JCTRL1',pin))
for pin in ['2','4','6']:
    # PTH input grounds connect directly to the first-inner plane.
    pass
nodes['HB'].append((47.8,22));nodes['VDD'].append((51.8,46))
for ref in ['JGH1','JGH2']:nodes['AC'].append(pos(ref,2))
nodes['AC'] += [(28,35),(62,35)]
# Route driver nets on B.Cu. Existing power and gate copper form obstacles.
step=.1;ox=9;oy=7;W=740;H=510
