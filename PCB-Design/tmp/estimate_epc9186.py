...
import csv
o=Path('reports/EPC9186-inductance')
rows=[]
for idx,length in enumerate([15,25,35,45],1):
 copper=1.256637*.1*length/.7
 rows.append([idx,length,.1,.7,round(copper,3),2,round(copper+2,3)])
with (o/'rough-gate-estimate.csv').open('w',newline='') as f:
 w=csv.writer(f);w.writerow(['position_near_to_far','assumed_route_mm','spacing_mm','effective_width_mm','planar_nH','transition_allowance_nH','estimated_total_nH']);w.writerows(rows)
report="""# EPC9186 rough loop-inductance estimate
...
"""
(o/'rough-inductance-estimate.md').write_text(report)
print(rows)